...
const ProviderRouteGuard = ({ children }: ProviderRouteGuardProps) => {
  ...
  const checkAuthentication = async () => {
    try {
      ...
      // Check if this is a mock token (hardcoded login)
      if (providerToken.startsWith('mock-worker-token-') || providerToken.startsWith('mock-')) {
        // Allow mock tokens for hardcoded login
        setIsAuthenticated(true);
        return;
      }

      // Verify session with Supabase for real providers
